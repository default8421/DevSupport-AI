...
class KnowledgeDocument(Base):
    """知识库文档元信息。内置运营资料与用户上传文档共用此表。

    tenant_id 为空串表示全局内置文档，对所有租户可见；非空为该租户私有。
    status 承载上传生命周期：pending → processing → published，失败为 failed。
    """
    ...
    source: Mapped[str] = mapped_column(String(16), default="builtin")  # builtin / upload
...
class UserMemory(Base):
    ...
    只存被识别为实体的结构化值，不存自由文本——用户说过的话不会被原样固化
    再回灌进 prompt，以此限制注入面。所有读写都必须带 tenant_id。
    """
