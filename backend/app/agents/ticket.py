...
async def create_from_context(
    *,
    ...
    args = {
        ...
        # 工单是持久化边界：证据里可能带着用户贴进对话的身份证/密钥
        #（工具循环会把模型填的参数一并记进证据），落库前统一脱敏。
        # 这里不能依赖 security_node——它只处理最终答复，且在建单之后才跑。
        "evidence": json.dumps(
            desensitize.desensitize_obj(evidence), ensure_ascii=False
        )[:2000],
        "ai_diagnosis": desensitize.desensitize_text(ai_diagnosis or "")[:2000],
    }
    # user_id / conversation_id 不进 args：它们由 ctx 提供（服务端拥有身份）
