...
_SYS = (
    "你是技术支持的意图识别器。判断用户问题的意图类型并抽取关键实体。\n"
    f"意图类型（只能选其一）：{INTENTS}\n"
    "- doc_qa: 询问文档/概念/用法/如何做/如何排查/错误码含义；也包括用户上传资料、某个项目/系统的架构与实现。"
    "包括「Webhook 回调如何排查」「签名怎么生成」「interview 的 RAG 怎么做」等。\n"
    ...
    "- chitchat: 问候、自我介绍、能力询问；或仅表示「想提问/已上传文档」但尚未给出具体问题。\n"
    "消歧规则：提到 429/限流→rate_limit；问「怎么排查/如何配置/是什么含义」的指导类→doc_qa；"
    "项目/上传文档里的具体技术问题→doc_qa，不要判成闲聊；"
    ...
    "  「interview 项目的 RAG 怎么做的」→ doc_qa\n"
    "  「你能干嘛」→ chitchat\n"
    "  「我上传文档了呀」→ chitchat\n"
    "  「我想提问关于 interview 项目的问题」→ chitchat\n"
...
_GREET = re.compile(
    r"^(你好|您好|嗨|哈喽|在吗|早上好|晚上好|你(能干嘛|能做什么|会什么|是谁)|能干什么)[啊呀么吗？?！!\s]*$"
)
_UPLOADED = re.compile(
    r"^我(已经|刚|刚刚)?上传(了|过)?(文档|资料|文件)[了呀啊呢。！!？?\s]*$"
)
_WANT_ASK = re.compile(
    r"^我想(要)?(提问|问一下|问问)(关于|有关)?.{0,40}(的问题)?[。！!？?\s]*$"
)
_SPECIFIC = re.compile(r"怎么|如何|为什么|为啥|是什么|哪些|怎样|几步|实现|架构|原理")


def is_meta_chitchat(query: str) -> bool:
    """问候、能力询问、仅宣布「想提问/已上传」——还没有可检索的具体问题。"""
    q = (query or "").strip()
    if not q:
        return False
    if _GREET.match(q) or _UPLOADED.match(q):
        return True
    return bool(_WANT_ASK.match(q) and not _SPECIFIC.search(q))
...
async def classify(query: str, history: list[dict] | None = None) -> dict:
    ...
    if is_meta_chitchat(query):
        return {
            "intent": "chitchat",
            "confidence": 0.95,
            "entities": {},
            "route": ROUTE_MAP["chitchat"],
            "tokens": 0,
        }
